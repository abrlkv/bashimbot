...
class Bashim:
    ...
    def _get_quote_data(self, html):
        ...
        all_quotes = quotes.find_all('article', attrs={'class': 'quote'})

        for quote in all_quotes:
            # Получаем id цитаты.
            quote_id = quote.find(
                'a', attrs={'class': 'quote__header_permalink'})
            if quote_id is not None:
                quote_id = quote_id.get('href').replace('/quote/', '')
                if quote_id:
                    # Получаем текст цитаты
                    quote_text = quote.find(
                        'div', attrs={'class': 'quote__body'})
    ...
    def get_random_quote(self):
        ...
        return random.choice(self._get_html('/random'))

    def get_quote(self, id):
        result = self._get_html('/quote/' + str(id))
        if result == []:
            return '404'
        elif result == ['error']:
            return 'error'
        else:
            return self._get_html('/quote/' + str(id))[0]
